...
void loop(){
  if(millis()%1==0){
    ...
    double PwmLeft  = outputPitch + outputYaw + moveSpeed + turnSpeed;
    double PwmRight = outputPitch - outputYaw + moveSpeed - turnSpeed;
  ...
  if(millis()%101==0){
    ...
    if(rxData=='W')       moveSpeed = 100; // maju gan
    else if(rxData=='A')  turnSpeed = -100; // mutero kiri
    else if(rxData=='S')  moveSpeed = -100; // mundur cak
    else if(rxData=='D')  turnSpeed = 100; // mutero kanan
